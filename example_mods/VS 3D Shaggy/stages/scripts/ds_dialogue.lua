...
local utf8 = require('utf8') or utf8;
...
function diag.ensureSkin(self)
	if (self.skin and self.skin.data) then
		return true;
	end
	if (self:loadSkin('default')) then
			...
			elseif (line:startsWith('#')) then
				...
				elseif (branchLvl > 0 and line:startsWith('#else if')) then
					...
					if (branch.fired) then
						...
						local _, condition = propBundle.readValue(line, 10);
	...
	elseif name == "Music_Zep" then
		...
		elseif value1 == "fadeIn" then
			...
			elseif value2 == "1" then
				...
				soundFadeIn('scary_menu', 1.5, 0, 1)
